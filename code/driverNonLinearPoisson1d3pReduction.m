...
ddu = @(t,mu,nu,xi) nu*exp(xi*t).*(xi*xi*(2+sin(mu*t)) + xi*mu*cos(mu*ti) + ...
...
[x,y] = solverFcn(a, b, K, v, g_te{1}, BCLt, BCLv(mu_te(1),nu_te(1),xi_te(1)), ...
    BCRt, BCRv(mu_te(1),nu_te(1),xi_te(1)));
u_te = zeros(size(y,1),Nte);  u_te(:,1) = y;
parfor i = 2:Nte
    [x,u_te(:,i)] = solverFcn(a, b, K, v, g_te{i}, BCLt, BCLv(mu_te(i),nu_te(i),xi_te(i)), ...
