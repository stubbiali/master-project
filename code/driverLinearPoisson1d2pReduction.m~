...
reducer = 'SVD';
sampler = {'rand'};
Nmu_v = [10 20 30 40 50]; 
Nnu_v = [10 20 30 40 50];
L = 1:25;  
Nte = 50;
root = '../datasets';
...
for s = 1:length(sampler)
    for i = 1:length(Nmu_v)
        for j = 1:length(Nnu_v)
            % Shortcuts
            if strcmp(sampler{s},'unif')
                Nmu = Nmu_v(i);  Nnu = Nnu_v(j);
            elseif strcmp(sampler{s},'rand')
            ...
            [x, mu_tr, nu_tr, u_tr, UL_xl] = reducerFcn(mu1, mu2, Nmu, ...
                nu1, nu2, Nnu, sampler{s}, max(L), ...
                solverFcn, a, b, K, f, BCLt, BCLv, BCRt);

            % Evaluate force field for snapshot values of $\mu$
            g_tr = cell(Nmu,1);
            for k = 1:Nmu
                g_tr{k} = @(t) f(t,mu_tr(k));
            end
            ...
            for l = 1:length(L)
                ...
                for k = 1:Nte
                    [x, alpha_te(:,k)] = ...
                        solverFcn(a, b, K, g_te{k}, BCLt, BCLv, BCRt, ...
                        nu_te(k), UL_xl(:,1:L(l)));
                end
                ...
                if strcmp(sampler{s},'unif')
                    filename = sprintf(['%s/LinearPoisson1d2p_%s_%s%s_a%2.2f_' ...
                        ...
                        'nu2%2.2f_K%i_Nmu%i_Nnu%i_N%i_L%i_Nte%i.mat'], ...
                        root, solver, reducer, sampler{s}, a, b, BCLt, BCLv, BCRt, ...
                        mu1, mu2, nu1, nu2, K, Nmu, Nnu, Nmu*Nnu, L(l), Nte);
                elseif strcmp(sampler{s},'rand')
                    filename = sprintf(['%s/LinearPoisson1d2p_%s_%s%s_a%2.2f_' ...
                        'b%2.2f_%s%2.2f_%s_mu1%2.2f_mu2%2.2f_nu1%2.2f_' ...
                        'nu2%2.2f_K%i_Nmu%i_Nnu%i_N%i_L%i_Nte%i.mat'], ...
                        root, solver, reducer, sampler{s}, a, b, BCLt, BCLv, BCRt, ...
                        mu1, mu2, nu1, nu2, K, Nmu, Nmu, Nmu, L(l), Nte);
                end
                UL = UL_xl(:,1:L(l));
                save(filename, 'x', 'mu_tr', 'nu_tr', 'u_tr', 'UL', 'alpha_tr', ...
