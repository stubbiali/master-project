...
filename = sprintf('%s/LinearPoisson1d1p_%s_%sunif_a%2.2f_b%2.2f_%s%2.2f_%s%2.2f_mu1%2.2f_mu2%2.2f_K%i_N%i_L%i_Nte%i.mat', ...
    root, solver, reducer, a, b, BCLt, BCLv, BCRt, BCRv, mu1, mu2, K, N, L, Nte);
load(filename);
...
% plot the accumulated error versus the number of training samples
...
% snapshots and the rank of the reduced basis: plot the accumulated error 
% versus number of basis functions for different number of snapshots.
...
for j = 1:length(N)
    semilogy(L', err_unif(:,j), marker_unif{j});
    hold on
...
% Define plot settings
str_leg = sprintf('Accumulated error $\\epsilon$ ($k = %i$, $n_{te} = %i$)', ...
    K, Nte);
title(str_leg)
xlabel('$l$')
ylabel('$\epsilon$')
