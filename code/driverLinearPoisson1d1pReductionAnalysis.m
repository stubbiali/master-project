...
% reduced solutions
filename = sprintf(['%s/LinearPoisson1d1p_%s_%sunif_a%2.2f_b%2.2f_%s%2.2f_' ...
...
% reduced solutions
filename = sprintf(['%s/LinearPoisson1d1p_%s_%srand_a%2.2f_b%2.2f_%s%2.2f_' ...
    '%s%2.2f_mu1%2.2f_mu2%2.2f_K%i_N%i_L%i_Nte%i%s.mat'], ...
    root, solver, reducer, a, b, BCLt, BCLv, BCRt, BCRv, mu1, mu2, K, N, L, ...
    Nte, suffix);
load(filename);
...
for i = 1:length(L)
    for j = 1:length(N)
        % Uniform sampling
        filename = sprintf(['%s/LinearPoisson1d1p_%s_%sunif_a%2.2f_b%2.2f_' ...
        ...
        % Random sampling
        filename = sprintf(['%s/LinearPoisson1d1p_%s_%srand_a%2.2f_b%2.2f_' ...
            '%s%2.2f_%s%2.2f_mu1%2.2f_mu2%2.2f_K%i_N%i_L%i_Nte%i%s.mat'], ...
            root, solver, reducer, a, b, BCLt, BCLv, BCRt, BCRv, mu1, mu2, ...
            K, N(j), L(i), Nte, suffix);
        load(filename);
...
% Load data
filename = sprintf(['%s/LinearPoisson1d1p_%s_%s%s_a%2.2f_b%2.2f_%s%2.2f_' ...
    '%s%2.2f_mu1%2.2f_mu2%2.2f_K%i_N%i_L%i_Nte%i%s.mat'], ...
