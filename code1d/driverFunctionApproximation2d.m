% driverFunctionApproximation2d Train a feedforward neural network to
% approximate any bi-dimensional scalar function.
...
f = @(x,y) (2*x - 5) + (y.^2)./(y-4);
a = -1;  b = 1;  c = -2;  d = 3;
Ntr_x = 50;  Ntr_y = 50;  Nva = 500;  Nte = 500;
H = 10;  nruns = 10;
transferFcn = 'logsig';
trainFcn = 'trainlm';
showWindow = true;
...
xtr = linspace(a,b,Ntr_x);  xtr = repmat(xtr,Ntr_y,1);  xtr = xtr(:);        
ytr = linspace(c,d,Ntr_y)';  ytr = repmat(ytr,Ntr_x,1);  ztr = f(xtr,ytr);  
xva = a + (b-a) * rand(Nva,1);  yva = c + (d-c) * rand(Nva,1);  zva = f(xva,yva);
xte = a + (b-a) * rand(Nte,1);  yte = c + (d-c) * rand(Nte,1);  zte = f(xte,yte);
...
for h = 1:length(H)
    ...
    for p = 1:nruns
        ...
        [net, tr] = train(net, [xtr' xva' xte'; ytr' yva' yte'], [ztr' zva' zte']);
