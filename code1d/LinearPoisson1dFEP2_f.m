% LinearPoisson1dFEP2_f Quadratic finite elements (FE-P2) solver for the linear 
...
% The "f" in the function name stands for "fast". Indeed, persistent variables 
% are used so to perform computations only when needed.
%
% [x, u] = LinearPoisson1dFEP2_f(a, b, K, f, BCLt, BCLv, BCRt, BCRv)
% [x, alpha] = LinearPoisson1dFEP2_f(a, b, K, f, BCLt, BCLv, BCRt, BCRv, UL)
% [x, u, alpha] = LinearPoisson1dFEP2_f(a, b, K, f, BCLt, BCLv, BCRt, BCRv, UL)
%
...
function [x, varargout] = LinearPoisson1dFEP2_f(a, b, K, f, BCLt, BCLv, BCRt, BCRv, varargin)
    ...
    % Compute the RHS
    if ~iscell(f)
        rhs = getLinearPoisson1dFEP2rhs_f(a, b, K, f, BCLt, BCLv, BCRt, BCRv);
    else
        N = length(f);
        rhs = zeros(K,N);
        for i = 1:N
            rhs(:,i) = getLinearPoisson1dFEP2rhs_f(a, b, K, f{i}, BCLt, BCLv, BCRt, BCRv);
        end
    end
